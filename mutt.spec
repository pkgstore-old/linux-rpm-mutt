...
%{__sed} -i -r 's/`$GPGME_CONFIG --libs`/"\0 -lgpg-error"/' configure

%{__install} -p -m644 %{SOURCE1} mutt_ldap_query
...
# We like GPG here.
%{__cat} contrib/gpg.rc >> \
  %{buildroot}%{_sysconfdir}/Muttrc

%{__grep} -5 "^color" contrib/sample.muttrc >> \
  %{buildroot}%{_sysconfdir}/Muttrc

%{__cat} >> %{buildroot}%{_sysconfdir}/Muttrc <<\EOF
...
%{__rm} -f %{buildroot}%{_sysconfdir}/*.dist
%{__rm} -f %{buildroot}%{_sysconfdir}/mime.types
# Disable mutt_dotlock program - remove the compiled binary.
%{__rm} -f %{buildroot}%{_bindir}/mutt_dotlock
%{__rm} -f %{buildroot}%{_bindir}/muttbug
%{__rm} -f %{buildroot}%{_bindir}/flea
%{__rm} -f %{buildroot}%{_mandir}/man1/mutt_dotlock.1*
%{__rm} -f %{buildroot}%{_mandir}/man1/muttbug.1*
%{__rm} -f %{buildroot}%{_mandir}/man1/flea.1*
%{__rm} -f %{buildroot}%{_mandir}/man5/mbox.5*
%{__rm} -f %{buildroot}%{_mandir}/man5/mmdf.5*
%{__rm} -rf %{buildroot}%{_pkgdocdir}

# Remove /usr/share/info/dir.
%{__rm} %{buildroot}%{_infodir}/dir

# Provide muttrc.local(5): the same as muttrc(5).
%{__ln_s} -f ./muttrc.5 %{buildroot}%{_mandir}/man5/muttrc.local.5
